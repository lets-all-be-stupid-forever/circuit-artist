...
static inline bool IsDigit(int key) {
  return key >= KEY_ZERO && key <= KEY_NINE;
}

void NumberModalOpen(Ui* ui) {
  ui->window = WINDOW_NUMBER;
  C.alive = 0;
  C.tlen = 0;
  C.txt[0] = '\0';
}

static int ParseModalNumber() {
  int n = -1;
...
void NumberModalUpdate(Ui* ui) {
  float dt = GetFrameTime();
  C.alive += dt;
  int key = GetCharPressed();
  int max_input_chars = MAX_TEXT_SIZE;
  while (key > 0) {
    if (IsDigit(key) && (C.tlen < max_input_chars)) {
