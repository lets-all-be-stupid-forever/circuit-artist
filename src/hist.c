...
void CmdUndo(Cmd *c, Hist *h)
{
  ...
  switch (c->action_type) {
    case ACTION_BUFF: {
      ...
      PyramidUpdateRect2(HIST_PYR_LVLS, h->buffer, AddOffsetToRect(r, c->offset));
      break;
    }
    case ACTION_SEL_CREATE: {
      if (c->sel_rect.width > 0) {
        if (c->paste_data.width == 0) {
          ...
          // The destination should be empty so a copy should suffice (no need to combine)
          RectangleInt r = {.x = 0, .y = 0, .width = h->selbuffer[0].width, .height = h->selbuffer[0].height};
      ...
      if (c->data_after.width > 0) {
        ...
        RectangleInt valid = GetCollisionRecInt(selrect, GetImageRect(h->buffer[0]));
        if (!IsRecIntEmpty(valid) && c->data_before.width > 0) {
          ...
          // buffer, since it was used for extraction of that buffer in the first
          // place...
          assert(c->data_before.width == valid.width);
          assert(c->data_before.height == valid.height);
          RectangleInt r = {.x = 0, .y = 0, .width = valid.width, .height = valid.height};
          CopyImage(c->data_before, r, &h->buffer[0], valid_offset);
          PyramidUpdateRect2(HIST_PYR_LVLS, h->buffer, AddOffsetToRect(r, valid_offset));
        }
      }
      break;
    }
    ...
    case ACTION_SEL_FLIP_H: {
      ...
      PyramidUpdateRect2(HIST_PYR_LVLS, h->selbuffer, GetImageRect(h->selbuffer[0]));
      break;
    }
  ...
  switch (c->action_type) {
    ...
    case ACTION_SEL_CREATE: {
      if (c->data_after.width > 0) {
        ...
        rdst.x = off.x;
        rdst.y = off.y;
        RectangleInt fixed_rdst = GetCollisionRecInt(rdst, GetImageRect(h->buffer[0]));
        // The scenario here is when the user drags the selection outside of the image
        // When we are deleting the selection, the data_before will be empty.
      ...
      if (c->sel_rect.width > 0) {
        ...
        if (c->paste_data.width == 0) {
          ...
          h->selbuffer[0] = GenImageFilled(c->sel_rect.width, c->sel_rect.height, BLANK);
          CopyImage(h->buffer[0], c->sel_rect, &h->selbuffer[0], (Vector2Int){0});
...
void HistActSelFill(Hist *h, Color fill_color)
{
  ...
  c->next = h->undo_hist;
  c->paste_data = GenImageFilled(h->selbuffer[0].width, h->selbuffer[0].height, fill_color);
