...
vec4 get_pixel(vec2 pos, sampler2D layer, int l) {
 ...
 // if (mode ==0) p=1;
  //if (mode != 0 && di > -(spi/2+1)) p = 0;
  if (di > -(spi/2+1)) p = 0;

  if (mode == -2) img_color1 = vec4(0);
  if (mode == 1 || mode == -3) {
    img_color1 = layer_color[l];
  }
  if (mode == 2) {
  ...
  /* tints interior with pattern p*/
  if (di<=-1) {
    img_color1 = p*img_color1;
  }

  /* exterior = black/transparent */
...
vec4 blend2(vec4 c1, vec4 c, int l) {
  ...
  if (l <= al) {
    ...
    if (c1.a>0.0 && c.a>0.95)  return vec4(0,0,0,1);
    return blend(c1, c);
  }
}
...
//  if (c3.a > 0 && layer_mode[3] != -2) c = blend2(c3, c, 3);
  ...
  for (int l =0; l < nl; l++) {
    if (l == al) continue;
    vec4 cc = drawlayer(l);
    if (cc.a > 0 && layer_mode[l] != -2) c = blend2(cc, c, l);
  }

  if (true) {
    vec4 ca = drawlayer(al);
    //vec3 finalColor = abs(backgroundColor - grayColor);
    //c.rgb = abs(ca.rgb  - c.rgb);
    // if (ca.a > 0.1) {
    //   if (c.a > 0.1) c = vec4(0);
    //   else c = ca;
    // }
