...
float get_mouse_dist() {
  vec2 pos = fragTexCoord;
  pos.y = 1.0 - pos.y;
   return length(pos * img_size - mouse);
}
...
vec4 get_pixel(sampler2D layer, int l) {
  ...
  // if (mode == 0) {
  //   img_color1 = (0.6 + 0.4*p) * img_color1;
  //   img_color1.a = 1.0;
  // }
  if (mode == 2) {
  ...
  if (via && ((l == al) || (l == al+1))) {
    ...
    bool down = (l == al);
    img_color1 = via_formula(img_color1, !down);
    return img_color1 ;
  }
...
#if 0
  if (l != al) {
    float md = get_mouse_dist();
    float f = smoothstep(80/sp.x,120/sp.x, md);
    return (1-f) * img_color1 +f * vec4(0);
  }
#endif
  return img_color1;
}
...
{
  vec4 c = vec4(0,0,0,1);
if (al ==1)c = vec4(0.1,0.1,0.1,1);
if (al ==2)c = vec4(0.2,0.2,0.2,1);
  vec4 c0 = drawlayer(0);
  vec4 c1 = drawlayer(1);
  vec4 c2 = drawlayer(2);
  vec4 c3 = drawlayer(3);
