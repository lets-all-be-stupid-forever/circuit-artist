...
// need 4 numbers:
// t0, t1, t2, t
// t0->t1 --> info entering nand. 
// t1->t2 --> nand "evaluation"
// t2->t2 --> info leaving nand
// What determines NAND speed? Mainly driver strength: Too much wire out = slower to activate.
//
...
void main()
{
  float nrot = phase.y;
    ...
    //if (r < 0.5 && r > 0.3) { color = color + vec4(p, p, p, 1.0); }
    float h = 1.0/18.0;
    // if (r2 < h) color += vec4(1);
    // else color += vec4(r2,r2,1-r2,0.5);

    // negative is towards the top!
    //if (rx > 0 && abs(ry) < h/2) color += vec4(1, 1, 0, 1);

    // pixel 0,0 is the center one
    // color = pixel(color, pos, h, vec2(4*h, 0), cnand3); // D
    // color = pixel(color, pos, h, vec2(-6*h, -2*h), cnand1); // A0
    // color = pixel(color, pos, h, vec2(-6*h, 2*h), cnand2); // A1
